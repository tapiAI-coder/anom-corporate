...
- **モバイルの描画安定化（2026-07-06）**: スマホでガタつく不具合の修正。**真因＝アプリ内ブラウザ（Instagram/LINEのWebView）特有のスクロール／ツールバー挙動と、慣性スクロールLenis＋毎フレームのスクロール連動演出（scrub）の競合**（Chrome/Safariの通常ブラウザでは出ないのが切り分けの鍵）。対策の中心＝**タッチ端末ではLenisと全scrub演出を止め、ネイティブスクロール＋CSS完成形の静止表示に統一する**: (1)`main.js`に`A.TOUCH`（`matchMedia("(hover: none),(pointer: coarse)")`）を追加し、Lenisは`!A.TOUCH`のPCのみ起動（＝毎フレームRAF・lagSmoothing(0)もPCだけ。タッチはGSAP既定のlag平滑化を残す） (2)`animations.js`のヒーローscrub一式（削り演出`.kezuru`・退場フェード`.hero-inner`・質感パララックス`.hero-tex`）を`if(!A.TOUCH)`で囲む→タッチでは削り線はCSS標準の完成形（rect width=104）で静止 (3)`setupInterlude`はタッチで早期return（乖離＝開いた完成形／合流＝一行の完成形がCSSで出る） (4)`setupFlowRail`はタッチで`--flow-p:1`固定 (5)`setupSeqScrub`は`A.TOUCH`も除外（タブレット含む）。**副次対策（同日先行分）**: `.nav.scrolled`の`backdrop-filter:blur`を900px以下で無効化＋不透明地色に／`.hero-copy .ch`の`will-change`は演出中だけJSで付け外し／`.band-track`の無限マーキーをIntersectionObserverで画面外停止／`document.fonts.ready.then(ScrollTrigger.refresh)`でフォント遅延swapのズレ補正。**検証時の注意**: (a)アプリ内ブラウザ特有の挙動はプレビューでも実機の通常ブラウザでも完全再現できない→最終判定はユーザーの実機（該当アプリ）に委ねる。(b)プレビューパネル非表示（`visibilityState:hidden`）だとCSS transition・GSAPのrAF tweenが凍結し`getComputedStyle`が古い値を返す→`.matches()`やCSSOM直読みで検証（詳細メモリ [[preview-hidden-transition-trap]]）
